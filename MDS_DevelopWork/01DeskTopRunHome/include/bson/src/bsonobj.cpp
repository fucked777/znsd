...
//
    ...
    FieldCompareResult compareDottedFieldNames(const string& l, const string& r)
    {
        ...
        logg() << "compareDottedFieldNames ERROR  l: " << l << " r: " << r
              << "  TOO MANY LOOPS" << endl;
        assert(0);
    ...
    char buffer[] = { 0x07,0x00,0x00,0x00,0x0a,0x00,0x00 };
	BSONObj staticNull = BSONObj(buffer, true);
#else
...
#pragma pack(1)
    ...
    BSONObj maxKey((const char *) &maxkeydata,true);
    ...
    BSONObj minKey((const char *) &minkeydata,true);
...
#pragma pack()
    ...
    void BSONObjBuilder::appendMinForType(const StringData& fieldName, int t) {
        ...
        logg() << "type not support for appendMinElementForType: " << t << endl;
