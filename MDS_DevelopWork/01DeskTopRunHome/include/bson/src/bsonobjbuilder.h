...
#pragma warning( disable : 4355 )
    ...
    template<typename T>
    class NOINLINE_DECL BSONFieldValue {
    public:
    ...
    */
    class BSOM_EXPORT BSONObjBuilder : boost::noncopyable {
    public:
        ...
        */
        BSONObjBuilder& appendDate(const StringData& fieldName, Date_t dt) {
            ...
            if( dt > 0 && dt <= 0xffffffff ) {
                static int n;
                if( n++ == 0 )
                    logg() << "DEV WARNING appendDate() called with a tiny "
    ...
    class NOINLINE_DECL BSONArrayBuilder : boost::noncopyable {
    public:
