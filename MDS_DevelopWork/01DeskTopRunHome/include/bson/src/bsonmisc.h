...
namespace bson {
    ...
    int NOINLINE_DECL getGtLtOp(const BSONElement& e);

    struct NOINLINE_DECL BSONElementCmpWithoutField {
        bool operator()( const BSONElement &l, const BSONElement &r ) const {
            return l.woCompare( r, false ) < 0;
        }
    };

    class NOINLINE_DECL BSONObjCmp {
    public:
    ...
    FieldCompareResult BSOM_EXPORT compareDottedFieldNames( const string& l , const string& r );
...
#define BSON_ARRAY(x) (( bson::BSONArrayBuilder() << x ).arr())
    ...
    */
	BSOM_EXPORT extern struct GENOIDLabeler { } GENOID;
    ...
    */
	BSOM_EXPORT extern struct DateNowLabeler { } DATENOW;

    /* Utility class to add the minKey (minus infinity) to a given attribute
       Example:
         cout << BSON( "a" << MINKEY ); // { "a" : { "$minKey" : 1 } }
    */
	BSOM_EXPORT extern struct MinKeyLabeler { } MINKEY;
	BSOM_EXPORT extern struct MaxKeyLabeler { } MAXKEY;

    // Utility class to implement GT, GTE, etc as described above.
    class BSOM_EXPORT Labeler {
    public:
        struct Label {
	...
	BSOM_EXPORT extern Labeler::Label GT;
	BSOM_EXPORT extern Labeler::Label GTE;
	BSOM_EXPORT extern Labeler::Label LT;
	BSOM_EXPORT extern Labeler::Label LTE;
	BSOM_EXPORT extern Labeler::Label NE;
	BSOM_EXPORT extern Labeler::Label SIZE;
    ...
    class BSOM_EXPORT BSONObjBuilderValueStream : public boost::noncopyable {
    public:
     ...
     */
    class BSOM_EXPORT BSONSizeTracker {
    public:
        BSONSizeTracker() {
            _pos = 0;
