...
namespace bson {
    ...
    class BSOM_EXPORT BSONObj {
    public:

        /** Construct a BSONObj from data in the proper format.
        * owned = whether this object owns this buffer.
        */
        explicit BSONObj(const char *msgdata,bool big = false) {
            _isbig = big;
          init(msgdata,big);
        }
        ...
        */
        ...
        int objsize() const
          {
            //std::cout << "objsize:: " << _isbig << std::endl;
#ifdef ARCH_IS_BIG_ENDIAN
            if(_isbig)
            {
                //std::cout << "int objsize()" << to_big(objdata());
                return to_big(objdata());
            }
            else
            {
                //std::cout << "objsize_big:: " << _isbig << std::endl;
...
#pragma pack()
    ...
    private:
        ...
        void init(const char *data,bool big = false) {
            _isbig = big;
            _objdata = data;
            if ( !isValid() )
                _assertInvalid();
    ...
    struct BSOM_EXPORT BSONArray : BSONObj {
