...
#ifndef HMPLDAPEXPR_P_H
#define HMPLDAPEXPR_P_H
...
public:
  ...
  /**
   * Creates an invalid HMPLDAPExpr object. Use with care.
   *
   * @see isNull()
   */
  HMPLDAPExpr();
  ...
  //!
  HMPLDAPExpr(const QString &filter);

  //!
  HMPLDAPExpr(const HMPLDAPExpr& other);

  HMPLDAPExpr& operator=(const HMPLDAPExpr& other);
  ...
  //!
  static bool query(const QString &filter, const HMPDictionary &pd);

  //! Evaluate this LDAP filter.
  bool evaluate(const HMPServiceProperties &p, bool matchCase) const;
...
private:
  ...
  //!
  HMPLDAPExpr(int op, const QList<HMPLDAPExpr> &args);

  //!
  HMPLDAPExpr(int op, const QString &attrName, const QString &attrValue);

  //!
  static HMPLDAPExpr parseExpr(ParseState &ps);

  //!
  static HMPLDAPExpr parseSimple(ParseState &ps);
  ...
  //! Shared pointer
  QSharedDataPointer<HMPLDAPExprData> d;

};


#endif // HMPLDAPEXPR_P_H
