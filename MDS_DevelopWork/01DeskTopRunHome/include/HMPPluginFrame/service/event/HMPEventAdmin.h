...
#ifndef HMPEVENTADMIN_H
#define HMPEVENTADMIN_H
 ...
 *
 */
struct HMPEventAdmin
{
  virtual ~HMPEventAdmin() {}
   ...
   *
   */
  virtual void postEvent(const HMPEvent& event) = 0;
   ...
   *
   */
  virtual void sendEvent(const HMPEvent& event) = 0;
   ...
   * following signature:
   * \code
   * someSignal(const HMPDictionary& props)
   * \endcode
   * where <code>props</code> will be used to construct a HMPEvent class which
   ...
   *
   * Slots should be registered with a property HMPEventConstants::EVENT_TOPIC.
   ...
   * \endverbatim
   *
   * Slots can also be registered with a property named HMPEventConstants::EVENT_FILTER.
   ...
   *
   * @throws HMPInvalidArgumentException If <code>subscriber</code> or <code>member</code> is 0
  ...
  virtual qlonglong subscribeSlot(const QObject* subscriber, const char* member,
                                  const HMPDictionary& properties, Qt::ConnectionType type = Qt::AutoConnection) = 0;
   ...
   */
  virtual bool updateProperties(qlonglong subscriptionId, const HMPDictionary& properties) = 0;

};


Q_DECLARE_INTERFACE(HMPEventAdmin, "org.commontk.service.event.EventAdmin")

#endif // HMPEVENTADMIN_H
