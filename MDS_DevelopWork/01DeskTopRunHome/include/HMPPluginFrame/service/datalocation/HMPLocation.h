...
#ifndef HMPLOCATION_H
#define HMPLOCATION_H
...
/**
 * A HMPLocation represents a QUrl which may have a default value, may be read only, may
 * or may not have a current value and may be cascaded on to a parent location.
 */
struct HMP_PLUGINFW_EXPORT HMPLocation
{
   ...
   */
  static const QString HMP_HOME_FILTER;
   ...
   */
  virtual HMPLocation* getParentLocation() const = 0;
   ...
   * @throws HMPIllegalStateException if the location's value is already set
   * @throws HMPRuntimeException if there was an unexpected problem while acquiring the lock
   */
   ...
   *
   * @exception HMPRuntimeException if there was an unexpected problem while acquiring the lock
   */
  virtual bool lock() = 0;
   ...
   * @throws HMPRuntimeException if there was an unexpected problem reading the lock
   */
   ...
   */
  virtual HMPLocation* createLocation(HMPLocation* parent, const QUrl& defaultValue, bool readonly) = 0;
   ...
   * to the returned data area.  An <code>HMPRuntimeException</code> will be thrown if
   ...
   * @throws HMPRuntimeException if the location URL is not already set
   */
  virtual QUrl getDataArea(const QString& path) const = 0;

};

Q_DECLARE_INTERFACE(HMPLocation, "org.commontk.service.datalocation.Location")
