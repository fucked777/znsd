...
#ifndef __HMPModelTester_h
#define __HMPModelTester_h
...
class HMPModelTesterPrivate;

/// \ingroup Core
/// HMPModelTester is a tool that tests any QAbstractItemModel
/// Most of the signals fired by the model set (HMPModelTester::setModel())
/// are connected to the tester that check their consistency with the 
/// model contents.
/// HMPModelTester is typically used when developing a new QAbstractItemModel
/// or during unit tests.
class HMP_CORE_EXPORT HMPModelTester: public QObject
{
  Q_OBJECT
...
public:
  ...
  /// setModel(...)
  explicit HMPModelTester(QObject *parent = 0);
  ...
  ///             verbose is true.
  HMPModelTester(QAbstractItemModel *model, QObject *parent = 0);

  ///
  /// Destructor
  virtual ~HMPModelTester();

  ///
  /// Set the model to be tested, the model must remain valid during 
  /// the life HMPModelTester.
...
protected:
  QScopedPointer<HMPModelTesterPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(HMPModelTester);
  Q_DISABLE_COPY(HMPModelTester);
};
