...
#ifndef __HMPCallback_h
#define __HMPCallback_h
...
///  QObject::connect(&slider, SIGNAL(valueChanged(int)),
///                   &callback, SLOT(invoke()));
class HMP_CORE_EXPORT HMPCallback : public QObject
{
  Q_OBJECT
public:

  HMPCallback(QObject * parentObject = 0);
  HMPCallback(void (*callback)(void * data), QObject * parentObject = 0);
  virtual ~HMPCallback();
  ...
  /// \note By default HMPCallback itself will be passed as callback data
  /// \sa setCallbackData
