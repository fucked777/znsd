...
#ifndef __HMPWorkflowStep_h
#define __HMPWorkflowStep_h
...
class HMPWorkflowStepPrivate;

/// \ingroup Core
/// \brief HMPWorkflowStep is the basis for a workflow step.
///
/// A workflow step is a placeholder for various states and transitions that are used in a
/// typical workflow. Such steps can be added to instances of the HMPWorkflow class.

class HMP_CORE_EXPORT HMPWorkflowStep
{

public:
  explicit HMPWorkflowStep();
  explicit HMPWorkflowStep(const QString& newId);
  virtual ~HMPWorkflowStep();

  /// Get the \a workflow associated with this step
  HMPWorkflow* workflow()const;
  ...
  QObject* HMPWorkflowStepQObject();

protected:

  explicit HMPWorkflowStep(HMPWorkflowStepPrivate * pimpl, const QString& newId);

  /// Set workflow
  void setWorkflow(HMPWorkflow* newWorkflow);
  ...
  /// </ul>
  ///
  /// The transition is of type HMPWorkflowTransition with the value
  /// HMPWorkflowTransitionType::ValidationTransition.
  HMPWorkflowIntrastepTransition* validationTransition()const;
  ...
  ///
  /// The transition is of type HMPWorkflowTransition with the value
  /// HMPWorkflowTransitionType::ValidationFailedTransition.
  HMPWorkflowIntrastepTransition* validationFailedTransition()const;
  ...
  /// of HMPWorkflowStep, or create a HMPWorkflowStep instance and use signals and slots, as
  ...
  virtual void onEntry(const HMPWorkflowStep* comingFrom, const HMPWorkflowInterstepTransition::InterstepTransitionType transitionType);
  ...
  /// in a subclass of HMPWorkflowStep, or create a HMPWorkflowStep instance and use signals and
  ...
  ///
  /// \param transitionType gives the type of the transition used to get to this step.
  virtual void onExit(const HMPWorkflowStep* goingTo,
  ...
  /// HMPWorkflowStep, following these instructions:
  /// <ul>
  ///   <li>invoke the superclass method HMPWorkflowStep::validateComplete(bool, const QString&)
  ...
  /// OR:
  ///
  /// 2) Create an instance of a HMPWorkflowStep then:
  /// <ul>
...
protected:
  QScopedPointer<HMPWorkflowStepPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(HMPWorkflowStep);
  Q_DISABLE_COPY(HMPWorkflowStep);
  friend class HMPWorkflow;        // For access to processingState, validationState, setWorkflow, validate
  friend class HMPWorkflowPrivate; // For access to invokeOn{Entry,Exit}Command, on{Entry,Exit}
};
