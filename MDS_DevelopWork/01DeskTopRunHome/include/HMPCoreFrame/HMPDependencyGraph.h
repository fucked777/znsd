...
#ifndef __HMPDependencyGraph_h
#define __HMPDependencyGraph_h
...
#else
#define HMP_CORE_EXPORT
#endif

#include <list>

class HMPDependencyGraphPrivate;

/// \ingroup Core
/// \class HMPDependencyGraph
/// \brief Class to implement a dependency graph, converted to STL instead of Qt.
class HMP_CORE_EXPORT HMPDependencyGraph
{
public:
  HMPDependencyGraph(int nvertices);
  virtual ~HMPDependencyGraph();
...
protected:
  HMPDependencyGraphPrivate* d_ptr;

private:

  // Intentionally disable copy semantics
  HMPDependencyGraph(const HMPDependencyGraph &);
  HMPDependencyGraph &operator=(const HMPDependencyGraph &);
};
