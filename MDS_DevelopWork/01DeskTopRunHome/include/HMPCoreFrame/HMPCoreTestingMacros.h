...
#ifndef __HMPCoreTestingMacros_h
#define __HMPCoreTestingMacros_h

#include "HMPCoreTestingUtilities.h"
...
#define CHECK_NULL(pointer) \
  ...
  if (!HMPCoreTestingUtilities::CheckNull(__LINE__,#pointer " is not NULL", pointerValue)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_NOT_NULL(pointer) \
  { \
  if (!HMPCoreTestingUtilities::CheckNotNull(__LINE__,#pointer " is NULL", (pointer))) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_EXIT_SUCCESS(actual) \
  { \
  if (!HMPCoreTestingUtilities::CheckInt(__LINE__,#actual " != EXIT_SUCCESS", (actual), EXIT_SUCCESS)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_INT(actual, expected) \
  { \
  if (!HMPCoreTestingUtilities::CheckInt(__LINE__,#actual " != " #expected, (actual), (expected))) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_POINTER(actual, expected) \
  { \
  if (!HMPCoreTestingUtilities::CheckPointer(__LINE__,#actual " != " #expected, (actual), (expected))) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_POINTER_DIFFERENT(actual, expected) \
  { \
  if (!HMPCoreTestingUtilities::CheckPointer(__LINE__,#actual " == " #expected, (actual), (expected), false)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_BOOL(actual, expected) \
  { \
  if (!HMPCoreTestingUtilities::CheckInt(__LINE__,#actual " != " #expected, (actual)?1:0, (expected)?1:0)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_STRING(actual, expected) \
  { \
  if (!HMPCoreTestingUtilities::CheckString(__LINE__,#actual " != " #expected, (actual), (expected))) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_STD_STRING(actual, expected) \
  ...
  if (!HMPCoreTestingUtilities::CheckString(__LINE__,#actual " != " #expected, a.c_str(), e.c_str())) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_QSTRING(actual, expected) \
  ...
  QString a = (actual); \
  QString e = (expected); \
  if (!HMPCoreTestingUtilities::CheckString(__LINE__,#actual " != " #expected, qPrintable(a), qPrintable(e))) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_STRING_DIFFERENT(actual, expected) \
  { \
  if (!HMPCoreTestingUtilities::CheckString(__LINE__,#actual " != " #expected, (actual), (expected), false)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_STD_STRING_DIFFERENT(actual, expected) \
  ...
  std::string a = (actual); \
  std::string e = (expected); \
  if (!HMPCoreTestingUtilities::CheckString(__LINE__,#actual " != " #expected, a.c_str(), e.c_str(), false)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_QSTRING_DIFFERENT(actual, expected) \
  ...
  QString a = (actual); \
  QString e = (expected); \
  if (!HMPCoreTestingUtilities::CheckString(__LINE__,#actual " != " #expected, qPrintable(a), qPrintable(e), false)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_QSTRINGLIST(actual, expected) \
  ...
  if (!HMPCoreTestingUtilities::CheckStringList(__LINE__,#actual " != " #expected, a, e)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
...
#define CHECK_QVARIANT(actual, expected) \
  ...
  QVariant a = (actual); \
  QVariant e = (expected); \
  if (!HMPCoreTestingUtilities::CheckVariant(__LINE__,#actual " != " #expected, a, e)) \
    { \
    return EXIT_FAILURE; \
    } \
  }
