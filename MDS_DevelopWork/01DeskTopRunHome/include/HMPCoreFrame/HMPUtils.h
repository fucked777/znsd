...
void HMP_CORE_EXPORT qListToSTLVector(const QStringList& list, std::vector<char*>& vector);

///
/// \ingroup Core
/// Convert a QStringList to a Vector of string
void HMP_CORE_EXPORT qListToSTLVector(const QStringList& list, std::vector<std::string>& vector);

///
/// \ingroup Core
/// Convert a Vector of string to QStringList
void HMP_CORE_EXPORT stlVectorToQList(const std::vector<std::string>& vector, QStringList& list);
...
QStringList HMP_CORE_EXPORT nameFilterToExtensions(const QString& nameFilter);

///
/// \ingroup Core
/// Convert a nameFilter to a list of file extensions:
/// "Images (*.png *.jpg *.tiff)", "Text (*.txt)" -> "*.png", "*.jpg", "*.tiff", "*.txt"
QStringList HMP_CORE_EXPORT nameFiltersToExtensions(const QStringList& nameFilters);

///
/// \ingroup Core
/// Convert a wildcar extension filter ("*.jpg") into a regular expression string
/// "*.jpg" -> ".*\\.jpg?$"
QString HMP_CORE_EXPORT extensionToRegExp(const QString& extension);
...
/// "*.jpg", "*.txt" -> "(.*\\.jpg?$|.*\\.txt?$)"
QRegExp HMP_CORE_EXPORT nameFiltersToRegExp(const QStringList& nameFilters);
...
/// See more cases in the test HMPUtilsSignificantDecimalsTest1
int HMP_CORE_EXPORT significantDecimals(double value, int defaultDecimals = -1);
...
/// See more cases in the test HMPUtilsOrderOfMagnitudeTest1
int HMP_CORE_EXPORT orderOfMagnitude(double value);
...
/// See more cases in the test HMPUtilsClosestPowerOfTenTest1
double HMP_CORE_EXPORT closestPowerOfTen(double value);
...
/// \sa QDir::rmdir
bool HMP_CORE_EXPORT removeDirRecursively(const QString & dirName);
...
/// \return <code>true</code> on success, <code>false</code> otherwise.
/// \sa QFile::copy
bool HMP_CORE_EXPORT copyDirRecursively(const QString &srcPath, const QString &dstPath);

///
/// \ingroup Core
/// Convert Qt::HANDLE to string
/// \sa Qt::HANDLE
QString HMP_CORE_EXPORT qtHandleToString(Qt::HANDLE handle);
...
/// bumping the minimum required Qt version for HMP.
qint64 HMP_CORE_EXPORT msecsTo(const QDateTime& t1, const QDateTime& t2);
