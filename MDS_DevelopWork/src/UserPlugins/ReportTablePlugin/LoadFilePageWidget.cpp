...
#include "ui_LoadFilePageWidget.h"
#include <QKeyEvent>
#include <QMessageBox>
LoadFilePageWidget::LoadFilePageWidget(QWidget* parent)
    : QWidget(parent)
    , ui(new Ui::LoadFilePageWidget)
{
    ui->setupUi(this);
    m_pDataModel = new LoadFileArrayModel(this);
    //    initMember();
    ...
    //设置每页显示行数按钮
    connect(ui->comboBox, &QComboBox::currentTextChanged, this, &LoadFilePageWidget::comboxCurtextChange);
}

LoadFilePageWidget::~LoadFilePageWidget() { delete ui; }

void LoadFilePageWidget::keyPressEvent(QKeyEvent* event)
{
    switch (event->key())
    {
...
//首页按钮按下
void LoadFilePageWidget::OnFirstButtonClick()
{
...
//末页按钮按下
void LoadFilePageWidget::OnLastButtonClick()
{
    if (m_pDataModel->GetArrayData().size() <= 0)
    {
        return;
...
//前一页按钮按下
void LoadFilePageWidget::OnPrevButtonClick()
{
    if (m_pDataModel->GetArrayData().size() <= 0)
    {
        return;
...
//后一页按钮按下
void LoadFilePageWidget::OnNextButtonClick()
{
    if (m_pDataModel->GetArrayData().size() <= 0)
    {
        return;
...
void LoadFilePageWidget::OnOkBtnPageButtonClick()
{
    //得到输入字符串
...
void LoadFilePageWidget::comboxCurtextChange(const QString& text)
{
    //得到输入字符串
    QString szText = text;
    //数字正则表达式
...
void LoadFilePageWidget::UpdateStatus()
{
    //刷新表格
